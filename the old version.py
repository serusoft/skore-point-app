...
<head>
  ...
  <style>
    ...
    * {
    ...
    .mobile-install-btn.primary {
      background: white;
      color: var(--primary);
    }

    .mobile-install-btn.primary:hover {
      background: #f0f0f0;
      transform: translateY(-2px);
    }

    .mobile-install-btn.secondary {
      background: rgba(255, 255, 255, 0.2);
      color: white;
      border: 1px solid rgba(255, 255, 255, 0.3);
    }

    .mobile-install-btn.secondary:hover {
    ...
    /* Form Elements - FIXED DROPDOWN STYLES */
    .form-group {
      margin-bottom: 20px;
      position: relative;
    }
    ...
    /* FIX: Style dropdown options to be visible on dark background */
    ...
    /* FIX: Style for data list dropdown */
    ...
    /* FIX: Marks entry section improvements */
    .marks-header {
      display: flex;
      justify-content: space-between;
      align-items: center;
    ...
    /* FIX: Analytics filters in same line for teachers */
    .analytics-filters {
      display: flex;
      flex-wrap: wrap;
      gap: 15px;
    ...
    .border-left i { transform: rotate(-90deg) scaleX(-1); }

    .border-right {
      top: 20px; bottom: 20px; right: 10px; width: 15px;
      flex-direction: column;
    }
    .border-right i { transform: rotate(90deg); }
    }
    ...
    .subject-assignment-item:hover {
      background: rgba(255, 255, 255, 0.05);
    }

    .subject-assignment-item input[type="checkbox"] {
      margin: 0;
    }

    .subject-assignment-item label {
      flex: 1;
      margin: 0;
      cursor: pointer;
    }
    ...
    @media (max-width: 768px) {
      ...
      /* FIX: Mobile responsive for analytics filters */
      .analytics-filters {
        flex-direction: column;
        align-items: stretch;
      }
    ...
    @media (min-width: 769px) {
      ...
      /* FIX: Keep marks and analytics buttons on same line for desktop */
      .marks-analytics-buttons {
        display: flex;
        gap: 15px;
        align-items: center;
      }
    ...
    /* FIX: Teacher action buttons on same line */
    .teacher-actions {
      display: flex;
      gap: 10px;
      margin-top: 15px;
  ...
  <div class="pwa-install-prompts">
    ...
    <div class="mobile-install-prompt" id="mobileInstallPrompt">
      ...
      <div class="mobile-install-body">
        Install the Skore Point app for a better experience. Get quick access, offline functionality, and push notifications.
      </div>
    ...
    <div class="container" id="schoolManagementSection">
      <div class="school-management">
            ...
            <div id="marksEntrySection">
              ...
              <div class="student-search-container mb-30">
                ...
                <div class="student-search-wrapper">
                  ...
                  <input type="text" id="studentSearch" class="student-search-input" placeholder="Type student name to search..." autocomplete="off">
        ...
        <div class="tab-content" id="reportsTab">
          ...
          <!-- Teacher Subject Analytics Section - FIXED LAYOUT -->
...
<script>
  ...
  let isAppInstalledCheckDone = false;
  let allSchoolsCache = [];
  ...
  const containers = {
    ...
    laterMobileBtn: document.getElementById('laterMobileBtn'),
    closeMobilePrompt: document.getElementById('closeMobilePrompt'),
  ...
  function setupPWA() {
    ...
    if ('serviceWorker' in navigator) {
      window.addEventListener('load', () => {
        navigator.serviceWorker.register('/testing/sw.js').then(registration => {
  ...
  function setupPWAInstallPrompts() {
  ...
  function checkIfAppInstalled() {
    if (isAppInstalledCheckDone) return;
  ...
  function showInstallPrompts() {
    console.log('Showing install prompts, isAppInstalled:', isAppInstalled);
    ...
    if (isMobileView) {
      setTimeout(() => {
        if (!isAppInstalled && deferredPrompt && !containers.mobileInstallPrompt.classList.contains('show')) {
          containers.mobileInstallPrompt.classList.add('show');
          console.log('Mobile install prompt shown');
        }
      }, 3000);
    } else {
      containers.desktopInstallBtn.style.display = 'flex';
      console.log('Desktop install button shown');
    }
  }
  ...
  async function installPWA() {
    console.log('Install PWA called, deferredPrompt:', !!deferredPrompt);
    ...
    if (!deferredPrompt) {
      ...
      checkIfAppInstalled();
      if (isAppInstalled) {
        alert('The Skore Point app is already installed on your device.');
        return;
      }
    ...
    try {
      ...
      if (choiceResult.outcome === 'accepted') {
        ...
        if (isMobileView) {
          alert('You can install Skore Point later from your browser menu. Look for "Add to Home Screen" or "Install App".');
        }
      }
      ...
      alert('Error installing app. Please try again.');
    }
  }
  ...
  async function loadSchoolData(schoolId) {
    try {
      ...
      if (schoolDoc.exists()) {
        ...
        console.log('User role in school:', currentUserData.role);
    ...
    if (filteredSchools.length > 0) {
      filteredSchools.forEach(school => {
        ...
        suggestion.innerHTML = `
          ...
          <span class="suggestion-code">${school.code}</span>
        `;
  ...
  async function handleJoinSchool(e) {
    ...
    try {
      ...
      // Clean school name (remove extra spaces, capitalize properly)
      ...
      querySnapshot.forEach(doc => {
        ...
        // Check if code matches AND name is similar (case-insensitive)
      ...
      if (!foundSchool) {
        ...
        showAlert('joinSchoolError', 
          'School not found. Please check:<br>' +
          '1. The school name is spelled correctly<br>' +
          '2. The 6-digit code is correct<br>' +
          '3. The school exists in the system<br><br>' +
          'Tip: Start typing the school name to see suggestions.', 
          'error');
        return;
      }
      ...
      // Add user to school's teacher list if not already there
      await firestoreModule.updateDoc(
      ...
      showAlert('joinSchoolSuccess', `Successfully joined ${currentSchool.name}! You can now access the school portal.`, 'success');
      ...
      showAlert('joinSchoolError', 
        'Error joining school. Please try again. If the problem persists, contact the school administrator.', 
        'error');
    }
  }
  ...
  async function handleRegisterSchool(e) {
    ...
    try {
      ...
      // Hide form after delay and refresh
      setTimeout(() => {
  ...
  function handleStudentSearch() {
    ...
    if (!classId || searchTerm.length === 0) {
  ...
  async function preloadMarksData() {
    try {
      ...
      const querySnapshot = await firestoreModule.getDocs(marksQuery);
  ...
  async function exportSubjectAnalytics() {
    try {
      ...
      // Sheet 1: Performance Overview with SKORE POINT header
      const overviewData = [
  ...
  async function generateReport() {
    ...
    try {
      ...
      if (studentId) {
        ...
        marksQuery = firestoreModule.query(
          firestoreModule.collection(firestoreDB, 'marks'),
          firestoreModule.where('schoolId', '==', currentSchool.id),
          firestoreModule.where('term', '==', term)
        );
  ...
  async function displayStudentReport(studentId, marks) {
    ...
    const averageGrade = calculateGrade(average, currentSchool.level);
    
    // Generate border icons
    const hIcons = Array(35).fill('<i class="fas fa-book-open"></i>').join('');
    const vIcons = Array(50).fill('<i class="fas fa-book-open"></i>').join('');
    const borderHTML = `
      <div class="report-border-container">
        <div class="report-border-side border-top">${hIcons}</div>
        <div class="report-border-side border-bottom">${hIcons}</div>
        <div class="report-border-side border-left">${vIcons}</div>
        <div class="report-border-side border-right">${vIcons}</div>
      </div>
    `;
  ...
  async function displayClassReport(classId, marksData) {
    ...
    marksData.forEach(mark => {
      ...
      if (count > 0) {
        ...
        studentAggregates.push(totalPoints);
        studentGrades.push(calculateGrade(average, currentSchool.level));
      }
    });
    
    const totalStudents = students.length;
    const avgScore = studentAverages.length > 0 ? Math.round(studentAverages.reduce((a, b) => a + b) / studentAverages.length) : 0;
    const avgGrade = calculateGrade(avgScore, currentSchool.level);
    const avgAggregate = studentAggregates.length > 0 ? Math.round(studentAggregates.reduce((a, b) => a + b) / studentAggregates.length) : 0;
    ...
    enhancedReportCard.innerHTML = `
      ...
      <table class="subject-table">
        ...
        <tbody>
          ...
          <tr>
            <td>Class Average Aggregate</td>
            <td>${avgAggregate}</td>
            <td>${getAggregateRemarks(avgAggregate, currentSchool.level)}</td>
          </tr>
          <tr>
  ...
  async function displaySchoolReport(marksData) {
    ...
    enhancedReportCard.innerHTML = `
      ...
      <table class="subject-table">
        ...
        <tbody>
          <tr>
            <td>School Average Score</td>
            <td>${avgScore}%</td>
            <td>${getPerformanceRemarks(avgScore)}</td>
          </tr>
          <tr>
            <td>School Average Grade</td>
            <td>${avgGrade}</td>
            <td>${getGradeRemarks(avgGrade)}</td>
          </tr>
          <tr>
            <td>School Average Aggregate</td>
            <td>${avgAggregate}</td>
            <td>${getAggregateRemarks(avgAggregate, currentSchool.level)}</td>
          </tr>
          <tr>
  ...
  async function exportAllStudentReportCards(classId, term) {
    ...
    try {
      ...
      const studentsWithMarks = classStudents.filter(student => {
        return marksData.some(mark => mark.studentId === student.id);
      });
      
      if (studentsWithMarks.length === 0) {
        hideLoadingOverlay();
        alert('No marks found for any student in this class. Please enter marks first.');
        return;
      }
      ...
      for (let i = 0; i < studentsWithMarks.length; i++) {
        ...
        const marksDoc = await firestoreModule.getDoc(
          firestoreModule.doc(firestoreDB, 'marks', `${student.id}_${term}`)
        );
        ...
        const marks = marksDoc.data();
        const studentClassObj = currentClasses.find(c => c.id === student.classId);
        ...
        await generateReportCardContent(doc, student, studentClassObj, marks, subjectScores, totalMarks, average, averageGrade, aggregate, division, reportTitle);
        
        doc.setFontSize(8);
        doc.setTextColor(150, 150, 150);
        doc.text(`Page ${i + 1} of ${studentsWithMarks.length}`, 105, 290, { align: 'center' });
      }
      
      const className = currentClasses.find(c => c.id === classId)?.name.replace(/\s+/g, '_') || 'Class';
      doc.save(`Report_Cards_${className}_${term}_${new Date().toISOString().slice(0, 10)}.pdf`);
      
      hideLoadingOverlay();
      alert(`Successfully exported ${studentsWithMarks.length} report cards!`);
  ...
  async function exportClassAnalysis() {
    ...
    try {
      ...
      // ENHANCED HEADER WITH SKORE POINT
      const classAnalysisData = [
        ...
        [''],
        ['ENHANCED STUDENT PERFORMANCE ANALYSIS'],
        ['Rank', 'Student Name', ...currentSubjects.map(s => s.name), 'Total Marks', 'Average Score', 'Average Grade', 'Aggregate', 'Final Grade/Division']
      ];
      ...
      XLSX.utils.book_append_sheet(wb, classAnalysisWs, 'Enhanced Class Analysis');
      
      const className = classObj ? classObj.name.replace(/\s+/g, '_') : 'Class';
      const fileName = `Enhanced_Class_Analysis_${className}_${term}_${new Date().toISOString().slice(0, 10)}.xlsx`;
      XLSX.writeFile(wb, fileName);
      
      hideLoadingOverlay();
      alert(`Enhanced Excel report "${fileName}" generated successfully!`);
  ...
  function getGradeRemarks(grade) {
    if (currentSchool.level === 'primary') {
      if (grade === 'D1' || grade === 'D2') return 'Distinction';
      if (grade === 'C3' || grade === 'C4' || grade === 'C5') return 'Credit';
      if (grade === 'C6') return 'Pass';
      if (grade === 'P7' || grade === 'P8') return 'Pass with Support';
      return 'Fail';
    } else {
  ...
  // Get aggregate remarks
  function getAggregateRemarks(aggregate, level) {
    if (level === 'primary') {
      if (aggregate <= 12) return 'Excellent';
      if (aggregate <= 24) return 'Good';
      if (aggregate <= 36) return 'Satisfactory';
...
IMPROVEMENTS:
✓ Fixed: School joining now works properly
✓ Fixed: Dropdown options visible on dark theme
✓ Fixed: Teachers can enter marks and analytics in same line
✓ Fixed: Firestore security rules permissions
✓ School Joining: Any authenticated user can join any school using name and code
✓ School Switching: Users can switch schools by joining another school
✓ Auto-formatting: School codes automatically convert to uppercase, spaces removed
✓ School Search: Real-time suggestions as you type school names
✓ Better Error Messages: Specific guidance when school not found

SCHOOL JOINING:
• Type school name - get suggestions as you type
• School codes automatically convert to UPPERCASE
• Spaces automatically removed from inputs
• Clear error messages with suggestions
• Can switch schools anytime

REPORT CARDS:
• Professional A4 design
• "SKORE POINT" branding with "A product of SERUSOFT"
• School logo prominently displayed
• Enhanced columns: Aggregate, Average Score, Average Grade
• Export as PDF with proper formatting

EXCEL EXPORTS:
• All files include "SKORE POINT - ANALYSIS GENERATED WITH SKORE POINT" header
• Professional formatting
• Enhanced student performance analysis
• Class rankings and statistics

MOBILE FEATURES:
• Bottom navigation with smooth scrolling
...
• Role management through school admin list
  ...
  window.deleteClass = async function(classId) {
    if (confirm('Are you sure you want to delete this class? All students in this class will also be deleted.')) {
      try {
        ...
        const studentsSnapshot = await firestoreModule.getDocs(studentsQuery);
        const deletePromises = [];
        
        studentsSnapshot.forEach(doc => {
          deletePromises.push(firestoreModule.deleteDoc(doc.ref));
        });
